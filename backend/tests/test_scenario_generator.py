...
def test_generate_strips_markdown_code_fences(monkeypatch):
    ...
    monkeypatch.setattr(gen, "_call_ollama", _canned(fenced))
...
def test_generate_retries_on_invalid_json_then_succeeds(monkeypatch):
    monkeypatch.setattr(gen, "_call_ollama", _canned("not json at all", json.dumps(VALID_DRAFT)))
...
def test_generate_retries_on_unknown_domain_then_succeeds(monkeypatch):
    ...
    monkeypatch.setattr(gen, "_call_ollama", _canned(json.dumps(bad), json.dumps(VALID_DRAFT)))
...
def test_generate_defaults_invalid_messiness_to_medium(monkeypatch):
    ...
    monkeypatch.setattr(gen, "_call_ollama", _canned(json.dumps(draft)))
