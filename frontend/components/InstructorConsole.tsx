...
export default function InstructorConsole({
  ...
  return (
    <main className="workspace">
      <h1>Instructor console — cohort {cohortId}</h1>
      ...
      <section className="panel">
        <h2>
          Students <button onClick={refresh} disabled={refreshing}>{refreshing ? "Refreshing…" : "Refresh"}</button>
        </h2>
        {instances.length === 0 ? (
            ...
            <tbody>
              {instances.map((instance) => (
                <tr key={instance.id}>
                  ...
                  <td>
                    <span className={`status-badge status-${instance.status}`}>{STATUS_LABEL[instance.status]}</span>
                  </td>
                  <td>
                  ...
                  <td>
                    <button onClick={() => viewSubmissions(instance.id)}>View submission</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {selectedInstanceId && (
        <section className="panel">
          <h2>Submission — student {instances.find((i) => i.id === selectedInstanceId)?.student_id}</h2>
          ...
          {submissions.map((submission) => (
            <div key={submission.id} className="submission-detail">
              <p className="submission-status">
                Status: <span className={`approval-badge approval-${submission.status}`}>{APPROVAL_LABEL[submission.status]}</span>
              </p>
